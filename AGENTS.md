...
사용자가 `오늘 코스 만들어줘`, `오늘의 코스 만들어줘`, `오늘 FIN DINING 만들어줘` 또는 같은 의미로 요청하면 다음을 수행한다.
...
사용자가 별도 주제를 붙이지 않고 `오늘 게시물 만들어줘`라고 요청하거나 Bull/Bear 시장 마감 게시물을 같은 뜻으로 요청하면 루트 코스 대신 `bull-bear-market-automation/AGENTS.md`와 `bull-bear-market-automation/automation-prompt.md`를 적용한다. KRX 정규장 마감 데이터로 당일 방향을 먼저 판정한 뒤 `bull` 또는 `bear` 중 하나만 만든다.
